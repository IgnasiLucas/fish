...
FASTQ=`pwd | sed "s/2015-12-16/2015-12-15/"`
...
for i in `seq 1 24`; do
   if [ ! -e pe/fastq/${NEWNAME[$i]}_R1.fastq ]; then
      ln -s $FASTQ/pe/${SAMPLE[$i]}_R1.fastq pe/fastq/${NEWNAME[$i]}_R1.fastq
   fi
   if [ ! -e pe/fastq/${NEWNAME[$i]}_R2.fastq ]; then
      ln -s $FASTQ/pe/${SAMPLE[$i]}_R2.fastq pe/fastq/${NEWNAME[$i]}_R2.fastq
   fi
   if [ ! -e se/fastq/${NEWNAME[$i]}.fastq ]; then
      ln -s $FASTQ/se/${SAMPLE[$i]}.fastq se/fastq/${NEWNAME[$i]}.fastq
   fi
done
...
if [ ! -e pe/params.txt ]; then
   ...
   sed -i '/## 9. /c\8                 ## 9. maxN: max number of Ns in reads (s2)' params.txt
...
cd pe
   pyrad -p params.txt -s 23 1> pyrad_s23.log 2> pyrad_s23.err &
cd ../se
   pyrad -p params.txt -s 23 1> pyrad_s23.log 2> pyrad_s23.err &
cd ..
